...
import controldaor.util.JsfUtil;
import controldaor.util.PaginationHelper;
...
public class ActorController implements Serializable {
    ...
    @EJB
    private session.ActorFacade ejbFacade;
    ...
    private void updateCurrentItem() {
        ...
        if (selectedItemIndex >= 0) {
            current = (Actor) getFacade().findRange(new int[]{selectedItemIndex, selectedItemIndex + 1}).get(0);
        }
    }
    ...
    public Actor getActor(java.lang.Short id) {
        return (Actor) ejbFacade.find(id);
    }
